...
async def create_session():
    url = f"http://{BASE_URL}/api/v1/agents/{AGENT_ID}/sessions"
...
async def send_message(session_id, message):
    # url = f"http://{BASE_URL}/api/v1/sessions/{session_id}/messages"
    url = f"ws://{BASE_URL}/ws"
    payload = {"role": "user", 
               "content": message,
               "metadata": "None"}
    try:
...
async def __add_message__(message, history, state): # {'text': '123', 'files': []}
    try:
        ...
        raise gr.Error("Unsupported Error: ", str(e))
